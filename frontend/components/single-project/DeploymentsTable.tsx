...
export default function DeploymentsTable({ projectId }: { projectId: string }) {
  ...
  const getStatusBadge = (status: Deployment["status"]) => {
    ...
    return (
      <Badge
        variant="outline"
        className={`${variants[status]} flex items-center gap-1.5 px-2.5 py-0.5`}
      >
  ...
  const mapStatus = (s: string) => {
    ...
    if (norm.includes("success") || norm.includes("succeeded"))
      return "Success";
    if (
      norm.includes("fail") ||
    ...
    async (page = 1) => {
      ...
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/projects/${projectId}/deployments?page=${page}&per_page=${ITEMS_PER_PAGE}`,
          {
            method: "GET",
            headers: {
        ...
        const mapped: Deployment[] = dtoList.map((d: any, idx: number) => ({
          ...
          type:
            d.build_version === data.current_version ? "Current" : "Previous",
        ...
        setTotalPages(
          data.pagination?.total_pages ??
            Math.ceil(
              (data.pagination?.total ?? mapped.length) / ITEMS_PER_PAGE,
            ),
        );
  ...
  const createDeployment = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/projects/${projectId}/generate-plan`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            ...
            >
              ...
              <div className="col-span-2 flex items-center">
                <div className="flex items-center gap-2 min-w-0">
                  <code
                    className="text-slate-400 bg-slate-800/50 px-2 py-1 rounded text-xs font-mono truncate max-w-[80px]"
                    title={deployment.commit_id || ""}
                  >
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-2">
          <div className="text-sm text-slate-400">
            ...
            of <span className="text-slate-200 font-medium">{total}</span>{" "}
            deployments
          </div>
