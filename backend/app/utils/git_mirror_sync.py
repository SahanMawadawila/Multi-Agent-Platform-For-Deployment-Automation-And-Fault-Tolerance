...
class GitMirrorSync:
    ...
    def sync_code(self, user_source_url, mirror_repo_name, env_variables: dict = None, source_access_token: str = None):
        ...
        try:
            ...
            if env_variables:
                ...
                # Check for all generated .env files in the repo tree
                env_files_to_add = []
                for root, dirs, files in os.walk(local_path):
                    ...
                    if '.env' in files:
                        ...
                        env_file_rel = os.path.join(rel_dir, '.env') if rel_dir != '.' else '.env'
                        env_files_to_add.append(env_file_rel.replace('\\', '/'))
                
                if env_files_to_add:
                    repo.index.add(env_files_to_add)
                    repo.index.commit("Add environment variables")
                    print("✅ Added .env files to repository:", env_files_to_add)
    ...
    def _create_env_file(self, repo_path: str, env_variables: dict):
        """
        Create a .env file in the repository directory or subdirectories for monorepos.

        Args:
            repo_path: Path to the local repository or subdirectory
            env_variables: Dictionary of {KEY: value} pairs or {DIR_NAME: {KEY: value}}
        """
        root_lines = []
        root_lines.append("# Auto-generated environment variables")
        root_lines.append("# Generated by Flow Pilot AI")
        root_lines.append("")
        ...
        for key, value in env_variables.items():
            if isinstance(value, dict):
                # It's a directory, so we create the sub-directory and its .env recursively
                sub_dir_path = os.path.join(repo_path, key)
                os.makedirs(sub_dir_path, exist_ok=True)
                self._create_env_file(sub_dir_path, value)
            else:
                has_root_vars = True
                # Handle values with spaces or special characters
                if isinstance(value, str) and (' ' in value or '"' in value or "'" in value):
                    escaped_value = value.replace('"', '\\"')
                    root_lines.append(f'{key}="{escaped_value}"')
                else:
