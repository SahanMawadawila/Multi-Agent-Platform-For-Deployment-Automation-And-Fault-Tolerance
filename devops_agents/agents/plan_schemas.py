...
class ApplicationComponent(BaseModel):
    ...
    project_type: Literal["node", "python", "springboot", "java", "go", "react", "vue", "angular", "vite", "nextjs"] = Field(..., description="Project type (e.g. backend language or frontend framework)")
    ...
    role: Literal["frontend", "backend", "worker", "api-gateway"] = Field("backend", description="Component role")
    ...
    build_output_dir: Optional[str] = Field("dist", description="Output directory for frontend builds (e.g., 'build' for CRA, 'dist' for Vite).")
